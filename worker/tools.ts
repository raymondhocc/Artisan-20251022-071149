...
export type ToolResult = WeatherResult | { content: string } | ErrorResult | { [key: string]: any };
...
const customTools = [
  ...
  },
  {
    ...
    function: {
      name: 'canvas_tool',
      description: 'Manipulates elements on the design canvas. Use this for adding, updating, deleting, or reordering visual elements like text, shapes, and images.',
      parameters: {
        type: 'object',
        properties: {
          action: {
            ...
            enum: ['add', 'update', 'delete', 'bringToFront', 'sendToBack']
          },
          elementId: {
            type: 'string',
            description: "The ID of the element to target for 'update', 'delete', or layering actions. If not provided for 'update', the last added or selected element is assumed."
          },
          elementType: {
            type: 'string',
            description: "The type of element to add.",
            enum: ['text', 'rectangle', 'circle', 'image', 'triangle', 'star']
          },
          properties: {
            type: 'object',
            description: "An object containing properties for the element to add or update.",
            properties: {
              x: { type: 'number' },
              y: { type: 'number' },
              width: { type: 'number' },
              height: { type: 'number' },
              color: { type: 'string', description: "A hex color code, e.g., '#FF0000' for red." },
              textContent: { type: 'string' },
              rotation: { type: 'number' },
              opacity: { type: 'number', description: "A value between 0 (transparent) and 1 (opaque)." },
              border: { type: 'string', description: "A CSS border string, e.g., '2px solid black'." },
              imageSrc: { type: 'string', description: "A URL for an image. For generic images, use Unsplash, e.g., 'https://source.unsplash.com/random/400x300/?sunset'." }
            }
          }
        },
        required: ['action']
...
export async function executeTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  try {
    switch (name) {
      ...
      case 'canvas_tool': {
        // This tool is handled on the client-side.
        // We just return the arguments to be processed by the frontend hook.
        return { ...args };
      }
      default: {
